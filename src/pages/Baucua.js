...
function Baucua() {
  ...
  const sortedListMusic = listMusic.slice().sort((a, b) => a.name.localeCompare(b.name));
  ...
  return (
    <div className={`Baucua${spinning ? ' spinning' : ''}`}>
      <div className='dia-container'>
        ...
        {showBat && <img className={`bat-cover${spinning ? ' shaking' : ''}`} alt='' src={bat} />}
      ...
      <div className='container'>
        <button className='btn' style={{ backgroundColor: 'lightblue' }} onClick={() => {getRandomImage(); audioRef.current.play();}}>
          Xốc
        </button>
        <button className='btn' style={{ backgroundColor: '#52D3FF' }} onClick={handleResetImages}>
          Reset
        </button>
        <button className='btn' style={{ backgroundColor: '#9370db' }} onClick={handleDapButton}>
          {text}
        </button>
      </div>

      <div className='sound'>
        <div style={{width:'150px',display:'flex',flexDirection:'row',alignItems:'center',cursor:'pointer',}} onClick={toggleMusic}>
            <img alt='' className='icon' src={icon}/>
            <p className='textP' style={{marginLeft:'15px',fontSize:'20px'}}>{textMusic}</p>
        </div>
        <select style={{border:'none',borderRadius:'10px'}} onChange={handleMusicChange} value={music}>
        ...
        <div style={{width:'180px', display:'flex',flexDirection:'row',alignItems:'center',cursor:'pointer'}} onClick={showModal}>
          <img style={{width:'30px',height:'30px'}} alt='' src={questionMark}/>  
          <p className='textP'  onClick={showModal} style={{marginLeft:'15px'}}>Hướng dẫn chơi</p>
        </div>
        <Modal width={1000} title="Hướng dẫn chơi" bodyStyle={customBodyStyle} centered open={isModalOpen} onCancel={handleOk} onOk={handleOk}>
